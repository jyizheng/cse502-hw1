...
int main(int argc, char* argv[]) {
	...
	// initialize the syscall emulation sub-system	
	sl_init(sys.get_max_elf_addr(), INIT_STACK_POINTER, (unsigned long)ram, RAM_SIZE, argc-1, &argv[1]);
	
	// (argc, argv) sanity check
	cerr << "===== Printing arguments of the program..." << endl;
	for (int j = 0; j <= argc-1; j++) {
		unsigned long guest_addr = INIT_STACK_POINTER + j * sizeof(uint32_t);
		...
		if (0 == j) {
			...
			// print the arg value using the guest's write syscall (testing, testing, 1-2-3 !!!)
			cerr << dec << "== argv[" << j-1 << "]: ";
			sl_syscall(4, 2, val, len, 0, 0, 0, 0, 0);
			cerr << endl;
		}
	}
	cerr << "==========================================" << endl;
...
#define TICK() do {                        \
	...
	while (main_time/ps_per_clock < 50*KILO && !Verilated::gotFinish()) {
		TICK();
	}
